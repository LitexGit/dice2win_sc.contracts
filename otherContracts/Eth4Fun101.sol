...
contract Eth4Fun101 {
    ...
    event HousePayment(address indexed beneficiary, uint amount);
    event SettleBetPayment(address indexed beneficiary, uint amount);
    event WithdrawPayment(address indexed beneficiary, uint amount);
    event RefundPayment(address indexed beneficiary, uint amount);
    ...
    function withdrawFunds(address beneficiary, uint withdrawAmount) external onlyOwner {
        ...
        emit WithdrawPayment(beneficiary, withdrawAmount);
    }
    ...
    function placeBet(uint betMask, uint modulo, uint commitLastBlock, uint commit, bytes32 r, bytes32 s) external payable {
        ...
        house.transfer(houseEdge);
        emit HousePayment(house, houseEdge);
    ...
    function settleBetCommon(Bet storage bet, uint reveal, bytes32 entropyBlockHash) private {
        ...
        uint transferAmount = diceWin == 0 ? 1 wei : diceWin;
        gambler.transfer(transferAmount);
        emit SettleBetPayment(gambler, transferAmount);
    }
    ...
    function refundBet(uint commit) external {
        ...
        emit RefundPayment(bet.gambler, amount);
    }
